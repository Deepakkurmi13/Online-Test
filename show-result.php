...
if(isset($_SESSION['username']) && isset($_GET['id']))
{
	...
	$userid = $_SESSION["id"];
    $id = $_GET['id'];	
	$tname ="user".$_SESSION['id'];
    $data33 = "SELECT * FROM $tname WHERE  contest_no=$id";
	$run77 = mysqli_query( $conn , $data33);
	$fun = mysqli_num_rows($run77 );
	if($fun == 0)
	{
		header("location:LMS");
	}
...
if($result == 1)
{
	...
	$tname ="user".$_SESSION['id'];
    $query13 = "SELECT * FROM $tname WHERE contest_no=$id";
    $run19 = mysqli_query($conn ,$query13);
	$run10 = mysqli_fetch_assoc($run19);
	$answer = $run10['answer']; 
	$no_attempt = $run10['no_attempt']; 
	$rightanswer = $run10['rightanswer']; 
	$wronganswer = $run10['wronganswer']; 
    $userans = json_decode($answer);
...
<table id="move">
<tr><td><p class="para6">Right :<?php echo $rightanswer;?></p></td>
<td><p id="line"></p></td>
<td><p class="para7">Wrong :<?php echo $wronganswer;?></p></td>
<td><p id="line"></p></td>
<td><p class="para8">No_Attempt :<?php echo $no_attempt;?></p></td></tr>
 ...
 //this a form line
    ...
    <?php
			...
			<tr style="width:auto;margin-top:-20px;"><td><span id="answer"><span id="answer1">Answer-</span><font id="answer2"><?php echo $data88[$i-1][$data[$i-1]-1];?></font></span></td></tr>
			<?php
